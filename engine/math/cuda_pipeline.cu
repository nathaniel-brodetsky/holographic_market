...
namespace holo::cuda
{
    ...
    CudaPipeline::CudaPipeline(
        ...
        MemoryArena &arena,
        int gpu_device)
        : lob_soa_{lob_soa}, ev_transfer_start_{cudaEventDisableTiming}, ev_transfer_done_{cudaEventDefault}, ev_compute_start_{cudaEventDisableTiming}, ev_compute_done_{cudaEventDefault}
    {
        (void)arena;
        init_gpu(gpu_device);
    ...
    {
        if (laplacian_.n_rows < 4 || laplacian_.nnz == 0)
            return;
    ...
    void CudaPipeline::record_signal(const ArbitrageSignal &sig)
    {
        ...
        {
            if (h_curl_out_[e] > max_curl)
                max_curl = h_curl_out_[e];
    ...
    void CudaPipeline::run_continuous(std::atomic<bool> &shutdown_flag)
    {
        while (!shutdown_flag.load(std::memory_order_acquire))
        {
            ...
            const uint64_t t0 = pipeline_now_ns();
            while (pipeline_now_ns() - t0 < k_pipeline_poll_ns && !shutdown_flag.load(std::memory_order_relaxed))
            {
