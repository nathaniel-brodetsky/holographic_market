...
static constexpr int   k_max_edges          = 4096;
static constexpr int   k_max_triangles      = 8192;
...
struct LobpcgWorkspace final
{
    ...
    {
        auto f = [](float *p) noexcept { if (p) cudaFree(p); };
        f(d_X); f(d_W); f(d_P); f(d_AX); f(d_AW);
        f(d_AP); f(d_R); f(d_gram); f(d_eigenvalues); f(d_spmv_buffer);
    }
