...
namespace holo
{
    ...
    public:
        LobSoA(
            ...
            std::size_t depth)
            : n_instruments_{n_instruments}, depth_{depth}, total_slots_{n_instruments * depth * k_sides}, bid_prices_{arena.alloc_span<float>(n_instruments * depth)}, ask_prices_{arena.alloc_span<float>(n_instruments * depth)}, bid_qtys_{arena.alloc_span<float>(n_instruments * depth)}, ask_qtys_{arena.alloc_span<float>(n_instruments * depth)}, seq_nos_{arena.alloc_span<std::uint32_t>(n_instruments * depth * k_sides)}, last_ts_ns_{arena.alloc_span<std::uint64_t>(n_instruments)}, stats_{arena.emplace<LobStats>()}
        {
            ...
            for (std::size_t i = 0U; i < seq_nos_.size(); ++i)
            {
                seq_nos_[i] = k_invalid_seq;
            }
            for (std::size_t i = 0U; i < last_ts_ns_.size(); ++i)
            {
                last_ts_ns_[i] = 0U;
            }
        }
        ...
        void apply(const LobUpdate &u) noexcept
        {
            ...
            const std::size_t seq_base =
                instr * depth_ * k_sides + depth * k_sides + static_cast<std::size_t>(u.side);
        ...
        [[nodiscard]] std::span<const float>
        raw_bid_price_array() const noexcept { return bid_prices_; }

        [[nodiscard]] std::span<const float>
        raw_ask_price_array() const noexcept { return ask_prices_; }

        [[nodiscard]] std::span<const float>
        raw_bid_qty_array() const noexcept { return bid_qtys_; }

        [[nodiscard]] std::span<const float>
        raw_ask_qty_array() const noexcept { return ask_qtys_; }
