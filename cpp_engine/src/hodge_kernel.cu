    ...
    float*       d_L1)
{
    ...
    if (row >= n_edges || col >= n_edges) return;

    float l1_lower = 0.0F;
    for (int v = 0; v < n_nodes; ++v) {
        l1_lower += d_B1[static_cast<size_t>(v) * static_cast<size_t>(n_edges) + static_cast<size_t>(row)]
                  * d_B1[static_cast<size_t>(v) * static_cast<size_t>(n_edges) + static_cast<size_t>(col)];
    }

    float l1_upper = 0.0F;
    for (int tri = 0; tri < n_triangles; ++tri) {
        l1_upper += d_B2[static_cast<size_t>(row) * static_cast<size_t>(n_triangles) + static_cast<size_t>(tri)]
                  * d_B2[static_cast<size_t>(col) * static_cast<size_t>(n_triangles) + static_cast<size_t>(tri)];
    }

    d_L1[static_cast<size_t>(row) * static_cast<size_t>(n_edges) + static_cast<size_t>(col)]
        = l1_lower + l1_upper;
}
    ...
    for (int k = 0; k < n_eigs; ++k) {
        if (fabsf(d_eigenvalues[k]) < threshold) {
            ...
            for (int f = 0; f < n_edges; ++f) {
                dot += d_eigenvectors[static_cast<size_t>(k) * static_cast<size_t>(n_edges) + static_cast<size_t>(f)]
                     * d_omega[f];
            }
            gamma_e += d_eigenvectors[static_cast<size_t>(k) * static_cast<size_t>(n_edges) + static_cast<size_t>(e)]
                     * dot;
        }
    }
    d_gamma[e] = gamma_e;
    ...
    float*       d_exact)
{
    ...
    for (int v = 0; v < n_nodes; ++v) {
        float col_sum = 0.0F;
        for (int f = 0; f < n_edges; ++f) {
            col_sum += d_B1[static_cast<size_t>(v) * static_cast<size_t>(n_edges) + static_cast<size_t>(f)]
                     * (d_omega[f] - d_gamma[f]);
        }
        node_pot += d_B1[static_cast<size_t>(v) * static_cast<size_t>(n_edges) + static_cast<size_t>(e)]
                  * col_sum;
    }
    d_exact[e]         = node_pot;
    ...
    while (pi < end_i && pj < end_j) {
        ...
        if (ni == nj && ni != i && ni != j) {
            ...
            for (int f = 0; f < n_edges; ++f) {
                ...
                if ((fs == i && fd == k_node) || (fs == k_node && fd == i)) e_ik = f;
                if ((fs == j && fd == k_node) || (fs == k_node && fd == j)) e_jk = f;
            }
    ...
    cudaStream_t     stream)
{
    ...
    int h_n_harmonic = 0;
    {
        ...
        CUDA_CHECK(cudaStreamSynchronize(stream));
        for (int i = 0; i < ne; ++i) {
            if (fabsf(h_evals[i]) < k_harmonic_threshold) ++h_n_harmonic;
        }
        pinned_free(h_evals);
    }
