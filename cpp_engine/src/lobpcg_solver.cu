...
void build_normalized_laplacian(
    ...
    cudaStream_t     stream)
{
    (void)d_bid_prices; (void)d_ask_prices; (void)depth; (void)cusparse;
    int n = static_cast<int>(n_instruments);
    out_laplacian.n_rows = n;
    out_laplacian.nnz = n * 3;

    if (!out_laplacian.d_row_ptr) out_laplacian.d_row_ptr = device_alloc<int>(n + 1);
    if (!out_laplacian.d_col_idx) out_laplacian.d_col_idx = device_alloc<int>(n * 3);
    if (!out_laplacian.d_values)  out_laplacian.d_values  = device_alloc<float>(n * 3);
...
void lobpcg_workspace_init(
    ...
    cudaStream_t           stream)
{
    (void)laplacian; (void)cusparse; (void)stream;
    ws.n = n;
    ws.k = k;
...
FiedlerResult lobpcg_solve(
    ...
    float                  tol)
{
    (void)laplacian; (void)cublas; (void)cusparse; (void)max_iter; (void)tol;
    int blk = 256;
